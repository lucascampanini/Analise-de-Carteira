...
import { getAnotacoes, postAnotacao } from "@/lib/api";
...
export function AnotacoesSection({ codigoConta }: { codigoConta: string }) {
  ...
  const load = () =>
    getAnotacoes(codigoConta).then(setAnotacoes).catch(() => {});

  useEffect(() => { load(); }, [codigoConta]);

  const salvar = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!texto.trim()) return;
    setSalvando(true);
    await postAnotacao(codigoConta, tipo, texto.trim());
    setTexto("");
    setSalvando(false);
    load();
  };
  ...
  const deletar = async (id: string) => {
    ...
    await fetch(`${API}/assistente/anotacoes/${id}`, { method: "DELETE" });
    load();
  };
            ...
            <div key={a.id} className="px-5 py-3 flex items-start gap-3 group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  {a.tipo} · {new Date(a.criado_em).toLocaleDateString("pt-BR", {
